pipeline {
    ...
    stages {
        stage('Build') {
            ...
            steps {
                sh "fpm --verbose -a amd64 -n teleservices-package -s python -t deb -v `echo ${VERSION}` --prefix /usr -d passerelle setup.py"
        ...
        stage('Push deb to buster apt repo') {
            ...
            steps {
                withCredentials([usernameColonPassword(credentialsId: 'nexus-teleservices', variable: 'CREDENTIALS'),string(credentialsId: 'nexus-url-buster', variable:'NEXUS_URL_BUSTER')]) {
                    sh ('curl -v --fail -u $CREDENTIALS -X POST -H Content-Type:multipart/form-data --data-binary @teleservices-package_`echo ${VERSION}`_amd64.deb $NEXUS_URL_BUSTER')
                }
            }
        }
